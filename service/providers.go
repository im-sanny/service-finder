...
func (s *provider) GetAll(page, limit int, filters map[string]string) ([]model.Provider, int64, error) {
	if page < 1 {
		page = 1
	}
	...
	providers, total, err := s.repo.GetAll(page, limit, filters)
	if err != nil {
		return nil, 0, fmt.Errorf("failed to get all providers: %w", err)
	}
